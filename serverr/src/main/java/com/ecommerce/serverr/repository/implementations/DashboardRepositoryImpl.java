...
public class DashboardRepositoryImpl implements DashboardRepository {
    ...
    public List<DashboardDTO> findVendasPorPeriodoEProduto(Integer livroId, LocalDate dataInicial, LocalDate dataFinal) {
        String buscarTodos = livroId != null ? "l.id = :livroId" : "l.id IS NOT NULL";

        String queryString = "SELECT l.titulo, (pitem.valor_unitario * pitem.quantidade_unitaria) as valorTotalPedidoItem, " +
                "pitem.quantidade_unitaria as itemQuantidade, p.data_pedido as dataPedido " +
                "FROM pedido_venda p " +
